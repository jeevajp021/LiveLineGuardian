...
# 1. Load the model (JSON format) and feature list (PKL)
# Re-using your logic from predict.py
model = xgb.Booster()
model.load_model(str(BASE_DIR / 'artifacts' / 'riskanalysis_model.json'))

with open(BASE_DIR / 'artifacts' / 'feature_names.pkl', "rb") as f:
...
# 4. Calculate SHAP values (TreeExplainer works directly with the Booster)
...
print("✅ Global risk factors plot updated for JSON model format.")
